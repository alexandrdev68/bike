...
class USER extends Dbase {
	public static $arUserFields = array (
			'name',
			'patronymic',
			'surname',
			'login',
	...
	public static $minpasswlen = 6;
	public static $maxpasswlen = 45;
	public static $currUserProperties = array ();
	public static $currRentProperties = array ();
	...
	static public function add($arFields, $filtr = true) {
		...
		if (! isset ( $arFields ['user_level'] )) {
			...
			if (CRYPT_SHA512 == 1) {
				...
				self::addMess ( TEMP::$Lang ['SYSTEM'] ['encrypt_system_invalid'] );
		}
	...
	static public function isAdmin() {
		if (@$_SESSION ['CURRUSER'] ['user_level'] == 552071 || @$_SESSION ['CURRUSER'] ['user_level'] == 552081)
			return true;
		else
			return false;
	}
	static public function isSuperAdmin() {
		if (@$_SESSION ['CURRUSER'] ['user_level'] == 552081)
			return true;
		else
			return false;
	}
	...
	static public function isClient() {
		if (@$_SESSION ['CURRUSER'] ['user_level'] == 4)
			return true;
		else
			return false;
	}
