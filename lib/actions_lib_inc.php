...
class Actions {
	...
	function registr_action_handler() {
		...
		if (mb_strlen ( $id_user, 'utf-8' ) < 3) {
			$response = array (
					'status' => 'error',
					'message' => TEMP::$Lang ['SYSTEM'] ['wrong_login'] 
			);
			return json_encode ( $response );
		} elseif (! isset ( $_POST ['uLevel'] )) {
			$response = array (
					'status' => 'error',
					'message' => TEMP::$Lang ['SYSTEM'] ['user_level_not_set'] 
			);
			return json_encode ( $response );
		} elseif (! preg_match ( '/^[0-9]{12,12}$/', $phone ) && @$_POST ['uLevel'] == 4) {
			$response = array (
					'status' => 'error',
					'message' => TEMP::$Lang ['SYSTEM'] ['wrong_phone'] 
			);
			return json_encode ( $response );
		} elseif (mb_strlen ( $pass, 'utf-8' ) < USER::$minpasswlen && @$_POST ['uLevel'] != 4) {
			$response = array (
					'status' => 'error',
					'message' => TEMP::$Lang ['SYSTEM'] ['password_to_small1'] . USER::$minpasswlen . TEMP::$Lang ['SYSTEM'] ['password_to_small2'] 
			);
			return json_encode ( $response );
		} elseif ($pass !== $repeat_pass && @$_POST ['uLevel'] != 4) {
			$response = array (
					'status' => 'error',
					'message' => TEMP::$Lang ['SYSTEM'] ['wrong_confirm_passw'] 
			);
			return json_encode ( $response );
		}
			...
			foreach ( $_FILES as $index => $foto ) {
				...
				if ($res !== false && ! is_array ( $res )) {
					...
					if ($res === false) {
						$response = array (
								'status' => 'error',
								'message' => TEMP::$Lang ['SYSTEM'] ['resize_error'] 
						);
						return json_encode ( $response );
					}
				} elseif (is_array ( $res ))
					...
					$response = array (
							'status' => 'error',
							'message' => TEMP::$Lang ['SYSTEM'] ['wrong_upload'] 
					);
					return json_encode ( $response );
				}
			} // foreach
	...
	function edit_user_handler() {
		...
		if (isset ( $_POST ['resStore'] ) || isset ( $_POST ['uLivePlace'] )) {
			if ($arUser === false)
				$response = array (
						'status' => 'bad',
						'message' => TEMP::$Lang ['SYSTEM'] ['wrong_sql_request'] 
				);
		...
		if (USER::change ( $arFields, $id, false )) {
			$response = array (
					'status' => 'ok',
					'message' => USER::lastMessage (),
					'uploaded_photo' => $imagepath == '' ? 'no' : 'yes' 
			);
		} else {
			$response = array (
					'status' => 'error',
	...
	function edit_bike_handler() {
		...
		if (mb_strlen ( $model, 'utf-8' ) < 2) {
			$response = array (
					'status' => 'error',
					'message' => TEMP::$Lang ['SYSTEM'] ['small_bike_name'] 
			);
			return json_encode ( $response );
		} elseif (! preg_match ( '/^([0-9]{1,})$|^[0-9]{1,}\\.(?:[0-9]{1,2})$/', $cost )) {
			$response = array (
					'status' => 'error',
					'message' => "Невірно введено суму" 
			);
			return json_encode ( $response );
		}
	...
	function login_action_handler() {
		$auth = USER::authorize ( array (
				'login' => Dbase::dataFilter ( $_POST ['uLogin'] ),
				'password' => $_POST ['uPassw'] 
		) );
		if ($auth !== false) {
			$response = array (
					'status' => 'ok',
		...
		if (isset ( $_COOKIE ['auth'] )) {
			...
			if (isset ( $_SESSION ['CURRUSER'] )) {
				...
				if ($token == $_COOKIE ['auth']) {
					...
					$response = array (
							'status' => 'authorised',
							'message' => "client has been authorised early, authorisation doesn't needed" 
					);
			...
			if ($arClientInfo ['status'] == 'ok') {
				...
				if ($arClientInfo ['count'] == 1) {
					...
					$response = array (
							'status' => 'ok',
							'type' => 'auth',
							'message' => TEMP::$Lang ['SYSTEM'] ['sms_sent_mess'] 
					);
					...
					$response = array (
							'status' => 'bad',
							'type' => 'auth',
							'message' => 'There are more than 1 clients by this phone' 
					);
					...
					$response = array (
							'status' => 'bad',
							'type' => 'auth',
							'message' => 'Client by this phone is not found' 
					);
				}
			} else {
				$response = array (
			...
			if ($arClientInfo ['status'] == 'ok') {
				if ($arClientInfo ['count'] == 0) {
					...
					$response = array (
							'status' => 'error',
							'type' => 'registration',
							'message' => TEMP::$Lang ['more_than_one_user_txt'] 
					);
			...
			if ($_POST ['smscode'] == $_SESSION ['sms_code']) {
				if (USER::client_authorize ( TEMP::getFromCash ( 'clientInfo' ) ) === true) {
					$response = array (
							'status' => 'ok',
							'type' => 'smsconfirm',
							'message' => TEMP::$Lang ['auth_succesfull_txt'] 
					);
				}
				;
			} else {
				$response = array (
						'status' => 'bad',
						'type' => 'smsconfirm',
						'message' => TEMP::$Lang ['sms_code_wrong_txt'] 
				);
			}
	...
	function get_users_list_handler() {
		if (USER::isAdmin ()) {
			...
			$response = array (
					'status' => 'error',
					'message' => TEMP::$Lang ['SYSTEM'] ['you_dont_access'] 
			);
			return json_encode ( $response );
		}
	}
	...
	function get_bikes_list_store_handler() {
		...
		if (@$_POST ['filter'] == 'in_store')
			$on_rent = 'no';
		elseif (@$_POST ['filter'] == 'on_rent')
			$on_rent = 1;
		...
		if ($arResult === false) {
			$response = array (
					'status' => 'bad',
					'message' => TEMP::$Lang ['SYSTEM'] ['wrong_sql_request'] . $sql 
			);
			return json_encode ( $response );
		}
		...
		foreach ( $arResult as $num => $bike ) {
			if ($bike ['foto'] != '')
				$arResult [$num] ['foto'] = 'upload/bikes/' . $bike ['foto'];
			if ($on_rent == 1) {
	...
	function user_delete_handler() {
		if (USER::delete ( @$_POST ['uid'] ) === true) {
			$response = array (
					'status' => 'ok',
					'mess' => TEMP::$Lang ['SYSTEM'] ['user_was_deleted'] 
			);
		} else
			$response = array (
					'status' => 'bad',
					'mess' => USER::lastMessage () 
			);
		return json_encode ( $response );
	}
	// ---------------------------------------
	function bike_delete_handler() {
		if (BIKE::delete ( @$_POST ['bid'] ) === true) {
			$response = array (
					'status' => 'ok',
					'mess' => TEMP::$Lang ['SYSTEM'] ['bike_was_deleted'] 
			);
		} else
			$response = array (
					'status' => 'bad',
					'mess' => TEMP::$Lang ['SYSTEM'] ['user_with_id2'] 
			);
		return json_encode ( $response );
	}
	// ---------------------------------------
	function add_bike_handler() {
		...
		if (mb_strlen ( $model, 'utf-8' ) < 2) {
			...
			$response = array (
					'status' => 'error',
					'message' => TEMP::$Lang ['SYSTEM'] ['number_uncorrect'] 
			);
			return json_encode ( $response );
		} elseif (BIKE::getInfo ( $number ) !== false) {
			$response = array (
					'status' => 'error',
					'message' => TEMP::$Lang ['SYSTEM'] ['bike_number_dublicat'] 
			);
			return json_encode ( $response );
		}
		// print_r(@$_FILES);die();
		...
		if (Dbase::$PDOConnection->exec ( $sql ) !== false) {
			$response = array (
					'status' => 'ok',
					'message' => TEMP::$Lang ['SYSTEM'] ['bike_add_success'] 
			);
		} else {
			$response = array (
					'status' => 'error',
					'message' => TEMP::$Lang ['SYSTEM'] ['error_add_bike'] 
			);
		}
	...
	function find_user_handler() {
		...
		$sql_where = 'WHERE (u.login LIKE "%' . @$_POST ['key'] . '%" OR u.name LIKE "%' . @$_POST ['key'] . '%" OR u.patronymic LIKE "%' . @$_POST ['key'] . '%" OR u.surname LIKE "%' . @$_POST ['key'] . '%" OR u.phone LIKE "%' . @$_POST ['key'] . '%") AND u.user_level = 4 LIMIT 20';
		...
		if ($arResult === false)
			return array (
					'status' => 'error',
					'message' => TEMP::$Lang ['bad_response_find'] 
			);
		foreach ( $arResult as $num => $user ) {
			$arResult [$num] ['properties'] = json_decode ( $user ['properties'], true );
		}
	...
	function add_klient_handler() {
		...
		if (mb_strlen ( $id_user, 'utf-8' ) < 3) {
			...
			$response = array (
					'status' => 'error',
					'message' => TEMP::$Lang ['SYSTEM'] ['photo_missing'] 
			);
			return json_encode ( $response );
		} elseif (USER::getInfo ( $id_user ) !== false) {
			$response = array (
					'status' => 'error',
					'message' => TEMP::$Lang ['SYSTEM'] ['mess_phone_was_created1'] . $id_user . TEMP::$Lang ['SYSTEM'] ['mess_login_was_created2'] 
			);
			return json_encode ( $response );
		}
		...
		if (BIKE::startRent ( $bike_id, $user_id, $rent_period, $added_json ) === true) {
			...
			if ($print_flag === true) {
				...
				if (isset ( $_POST ['bike_action'] ) && $_POST ['bike_action'] != 'false')
					$_SESSION ['PRINT'] ['action_ofert'] = true;
				else
					$_SESSION ['PRINT'] ['action_ofert'] = false;
				$response ['print'] = 'yes';
	...
	function stop_rent_handler() {
		...
		if (@$arInfo ['rent_prop'] ['added'] > 0)
			$added += BIKE::$added * 100;
		...
		if ($arStop !== false) {
			$response = array (
					'status' => 'ok',
					'stopTime' => $arStop ['time_stop'],
					'rent_amount' => (( int ) $arStop ['amount'] / 100),
					'fullInfo' => $arInfo,
					'message' => TEMP::$Lang ['SYSTEM'] ['stop_rent_sucess'] 
			);
		} else
			$response = array (
					'status' => 'bad',
	...
	function get_bike_rents_for_date_handler() {
		...
		if ($arBookings !== false) {
			...
			return json_encode ( array (
					'status' => 'bad',
					'message' => TEMP::$Lang ['SYSTEM' ['error_get_bookings']] 
			) );
		}
	}
	// ---------------------------------------
	...
	function accept_stores_handler() {
		foreach ( @$_POST ['accepted'] as $store ) {
			$response = array (
					'status' => 'ok',
					'message' => TEMP::$Lang ['SYSTEM'] ['store_changed_nothing'] 
			);
			...
			if ($store ['adress'] == '')
				...
				if (Dbase::$PDOConnection->exec ( $sql ) !== false) {
					$response = array (
							'status' => 'ok',
							'message' => TEMP::$Lang ['SYSTEM'] ['store_changed_success'] 
					);
				} else {
					$response = array (
							'status' => 'error',
							'message' => TEMP::$Lang ['SYSTEM'] ['error_change_store'] 
					);
				}
			}
		}
		...
		foreach ( @$_POST ['deleted'] as $store ) {
			...
			if ($store ['store_id'] != 'new') {
				...
				if (Dbase::$PDOConnection->exec ( $sql ) !== false) {
					$response = array (
							'status' => 'ok',
							'message' => TEMP::$Lang ['SYSTEM'] ['store_deleted_success'] 
					);
				} else {
					$response = array (
							'status' => 'error',
							'message' => TEMP::$Lang ['SYSTEM'] ['error_delete_store'] 
					);
				}
			}
		}
	...
	function period_report_handler() {
		...
		if ($to - $from > 2764800) {
			$response = array (
					'status' => 'bad',
					'message' => TEMP::$Lang ['SYSTEM'] ['period_to_big'] 
			);
		} else {
			$arRents = BIKE::getRentsFromPeriod ( $from, $to, $store_id );
			$response = array (
	...
	function cancel_rents_handler() {
		foreach ( @$_POST ['cancel'] as $rent ) {
			...
			if (Dbase::$PDOConnection->exec ( $sql ) !== false) {
				$response = array (
						'status' => 'ok',
						'message' => TEMP::$Lang ['SYSTEM'] ['rent_canceled_success'] 
				);
			} else {
				$response = array (
						'status' => 'error',
						'message' => TEMP::$Lang ['SYSTEM'] ['error_cancel_rent'] . $sql 
				);
			}
		}
	...
	function recalc_fact_handler() {
		if (@$_POST ['fact_time'] != '' && @$_POST ['rent_id'] >= 0) {
			...
			if (Dbase::$PDOConnection->exec ( $sql ) !== false) {
				...
				$response = array (
						'status' => 'error',
						'message' => TEMP::$Lang ['SYSTEM'] ['error_recalc_rent'] . $sql 
				);
			}
		} else
			$response = array (
					'status' => 'error',
					'message' => TEMP::$Lang ['SYSTEM'] ['error_recalc_rent'] 
			);
	...
	function find_action_user_handler() {
		if ($_SESSION ['CODE_COUNT'] > 4) {
			return json_encode ( array (
					'status' => 'bad',
					'message' => TEMP::$Lang ['txt_counter_code_too_big'] 
			) );
		}
		...
		if (! $arRes) {
			...
			return json_encode ( array (
					'status' => 'bad',
					'message' => TEMP::$Lang ['txt_sms_code_not_found'] 
			) );
		}
		...
		return json_encode ( array (
				'status' => "ok",
				'u_pos' => $user_position,
				'actions_count' => $action_users_count,
				'u_info' => $arRes [0],
				'leaders' => $arRes1,
				'upper' => $arRes3 
		) );
	...
	function send_sms_handler() {
		...
		if ($client_id == '' || $client_phone == '') {
			return json_encode ( array (
					'status' => 'bad',
					'message' => 'bad dataset for request' 
			) );
		}
		...
		if ($arRet ['status'] === true) {
			$arResponse = array (
					'status' => 'ok',
					'message' => TEMP::$Lang ['txt_sms_send_success'] 
			);
		} else {
			$arResponse = array (
					'status' => 'bad',
					'message' => TEMP::$Lang ['txt_sms_send_bad'] 
			);
		}
	...
	// ---------------------------------------
	function get_user_rent_history_handler() {
		$client_id = ( int ) Dbase::dataFilter ( $_POST ['user_id'] );
		$db = new Dbase ();
		...
		$sql = "SELECT 
				 ...
				 LEFT OUTER JOIN store s ON r.store_finish = s.id
				 WHERE klient_id = {$client_id} AND r.amount != -1 ORDER BY r.time_start ASC LIMIT 500";
		
		$arRes = $db->getArray ( $sql, true );
		...
		foreach ( $arRes as $num => $item ) {
			$rent_period_string = '';
			$summ += $arRes [$num] ['amount'];
			$arRes [$num] ['amount'] = $arRes [$num] ['amount'] / 100;
			$RentPeriod = BIKE::getTimeBetween($arRes [$num] ['time_start'], $arRes [$num] ['time_end']);
			//print_r($RentPeriod);
			$rent_period_string .= $RentPeriod['days'] == 0 ? '' : "{$RentPeriod['days']} д. ";
			$rent_period_string .= $RentPeriod['hours'] == 0 ? '' : "{$RentPeriod['hours']} год. ";
			$rent_period_string .= $RentPeriod['minutes'] == 0 ? '' : "{$RentPeriod['minutes']} хв. ";
			$rent_period_string .= $RentPeriod['seconds'] == 0 ? '' : "{$RentPeriod['seconds']} сек. ";
			$arRes [$num] ['rent_period'] = $rent_period_string;
			$arRes [$num] ['time_start'] = date ( 'd.m.Y H:i', $item ['time_start'] );
			$arRes [$num] ['time_end'] = date ( 'd.m.Y H:i', $item ['time_end'] );
		}
