<?php
	...
	$sql1 = "SELECT `b`.`id`, 
					...
					`b`.`foto`,
					`b`.`serial_id`,
					`s`.`adress` FROM `bikes` `b` LEFT OUTER JOIN `store` `s` ON `s`.`id` = `b`.`store_id` WHERE `b`.`id` >= 0 AND `b`.`on_rent` = 'no' LIMIT 300";
	...
	$sql2 = "SELECT `b`.`id`, 
					...
					`r`.`project_time`,
					`r`.`klient_id`,
					`s`.`adress` FROM `bikes` `b` LEFT OUTER JOIN `store` `s` ON `s`.`id` = `b`.`store_id` LEFT OUTER JOIN `rent` `r` ON `b`.`id` = `r`.`bike_id` WHERE `b`.`id` >= 0 AND `b`.`on_rent` = `r`.`id` LIMIT 100";

	$arBikesInStore = $db->getArray($sql1);


	//print_r($arBikesInStore);


	$arBikesOnRent = $db->getArray($sql2);
